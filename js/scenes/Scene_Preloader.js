...
function Scene_Preloader(){
	...
	// Border/frame for the recursive screen (cuadrito)
	var streamBorder = new PIXI.Graphics();
		var streamScale = 0.38;
		var streamX = 722;
		var streamY = 359;
		var streamW = Game.width * streamScale;
		var streamH = Game.height * streamScale;
		streamBorder.lineStyle(4, 0x333333, 1);
		streamBorder.beginFill(0x111111, 1);
		streamBorder.drawRoundedRect(
					streamX - streamW/2 - 8,
					streamY - streamH/2 - 8,
					streamW + 16,
					streamH + 16,
		...
		streamBorder.endFill();
		Game.stage.addChild(streamBorder);

	self.stream = new PIXI.Sprite();
		self.stream.x = streamX;
		self.stream.y = streamY;
		self.stream.scale.x = self.stream.scale.y = streamScale;
